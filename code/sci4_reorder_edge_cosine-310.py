...
print('\033[0;33mLoading Sentence Transformers...\033[0m')
...
def load_required_files_and_display_data() -> Union[pd.DataFrame, list, pd.DataFrame, list, list, list, dict, int]:
    """
    ...
    Returns:
        df, all_sent, qtype_df, qtypes, mcq, finalmcq, finalpara, adjustedIndex
    """
    ...
    return df, all_sent, qtype_df, qtypes, mcq, finalmcq, finalpara, adjustedIndex
...
def removepun(stringinp: str) -> str:
    '''
    Remove punctuation symbols for more efficient processing and to avoid
    "false negatives", i.e., something being marked as incorrect only due to
    a punctuation mark being off.

    Arguments:
        stringinp (str): The input whose punctuation marks are to be removed

    Returns:
        stringinp (str): Cleaned input string
    '''
...
def correct_and_wrong_match_analysis(qtypes: list, 
                                     mcq: list, finalmcq: list, finalpara: dict, 
                                     all_sent: list, adjustedIndex: int, 
    ...
    Arguments:
        qtypes (list): A list of the question types (what, which, how, etc.)
        ...
        all_sent (list): A list of all sentences
        adjustedIndex (int): Obtained from load_required_files_and_display_data()
        model (SentenceTransformer): The Sentence Transformer model being used
    ...
    Returns:
        A dictionary, containing:
            ...
            "edge_qtype": Question type of questions matched using edge matching
            "matchedques": A list of tuples of Paragraph Number and Question Number for the correctly matched question.
            "wrong": A list consisting of Paragraph Number, Question Number, Question, Correct Answer and 
                     Incorrectly Predicted Answer for the incorrectly matched question.
            "curr_corr": 
            "corr_qna": A list of tuples storing the Index, Question and Answer of correctly matched question
            "incorr_qna": A list of tuples storing the Index, Question and Predicted Answer and Actual Answer of
                          incorrectly matched question
            "correct_match": A list of scores for the correctly matched questions.
            "correct_score": A list of curr_corr (the exact elements that match between the paragraph and the answer) 
                           the correctly matched questions.
            "paras": List of paragraph numbers for the correctly matched questions.
            "wrong_match": A list of scores for the incorrectly matched questions.
            "wrong_score": A list of incurr_corr (the exact elements that match between the paragraph and the answer) 
                           the incorrectly matched questions.
            "wrong_paras": A list of paragraph numbers for the incorrectly matched questions.
    """

    print('\033[1;33mPerforming Correct and Wrong Match Analysis...\033[0m')
    edge_match = []
    pred_dict = {}
    edge_qtype = {}
    ...
    # The first step is to initialize empty lists and variables to keep track of
    # matched questions, wrong answers, current answer score and current correct
    # matches.
    matchedques = []
    wrong = []
    ...
    corr_qna = []
    incorr_qna = []
    logging.info('Starting logs for Correct and Wrong Match Analysis')
    # Next step is to open a connection to the CoreNLP server for processing text
    ...
    with CoreNLPClient(properties = CUSTOM_PROPS,
        ...
        for ques_dict in finalmcq:
            ...
            for questions in ques_dict["Answers"]:
                ...
                logging.info(f"Question No.:{count}, Paragraph No.: {para_no}\n")
                '''
                #3
                '''
                for ques in questions:
                    ...
                    # If the current question (x) has already been correctly matched
                    # (in 'matchedques'), it skips the rest of the loop.
                    if x in matchedques:
                        print(f"Already matched: {x}")
                        logging.info(f"Already matched: {x}\n")
                        continue
                    else:
                        ...
                        print(f"\033[1;32m\nMCQ option. ::::::::: {ans_no}   Framed answer::::::::: {qna['Answers'][ans_no-1]}\033[0m")
                        logging.info(f"\nMCQ option. ::::::::: {ans_no} Framed answer::::::::: {qna['Answers'][ans_no-1]}\n")
                        ...
                        score += common_no(finalpara[para_no]["nsubj"],
                                        ques["nsubj"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["nsubj"],
                                                    ques["nsubj"])[1])
                        score += common_no(finalpara[para_no]["dobj"],
                                        ques["dobj"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["dobj"],
                                                    ques["dobj"])[1])
                        score += common_no(finalpara[para_no]["vprep"],
                                        ques["vprep"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["vprep"],
                                                    ques["vprep"])[1])
                        score += common_no(finalpara[para_no]["nprep"],
                                        ques["nprep"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["nprep"],
                                                    ques["nprep"])[1])
                        score += common_no(finalpara[para_no]["verb"],
                                        ques["verb"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["verb"],
                                                    ques["verb"])[1])
                        score += common_no(finalpara[para_no]["prep"],
                                        ques["prep"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["prep"],
                                                    ques["prep"])[1])
                        score += common_no(finalpara[para_no]["adjv"],
                                        ques["adjv"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["adjv"],
                                                    ques["adjv"])[1])
                        score += common_no(finalpara[para_no]["subj_obj"],
                                        ques["subj_obj"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["subj_obj"],
                                                    ques["subj_obj"])[1])
                        score += common_no(finalpara[para_no]["noun"],
                                        ques["noun"])[0]
                        correctmatch.append(common_no(finalpara[para_no]["noun"],
                                                    ques["noun"])[1])
                        ...
                        correctmatch.append(common_no(m, j)[1])

                        print(f"\033[1;32mThe Total Score is {score}\033[0m")
                        logging.info(f"The Total Score is {score}\n")
                        '''
                        #6
                        '''
                        if score > max_score:
                            max_score = score
                            correct_ans = ans_no
                            curr_corr = correctmatch
                        elif score == max_score:
                            prev_ans_score = cosincode(para_no, count, correct_ans, all_sent, mcq, adjustedIndex, model)
                            curr_ans_score = cosincode(para_no, count, ans_no, all_sent, mcq, adjustedIndex, model)
                            if curr_ans_score > prev_ans_score:
                                correct_ans = ans_no

                # If the current question (x) is matched, skip the processing
                if x in matchedques:
                    pass
                else:
                    print(f"\033[1;32mPara No.:{para_no} Question No.: {count} Correct Ans (predicted option): {correct_ans}\033[0m")
                    logging.info(f"Para No.:{para_no} Question No.: {count} Correct Ans (predicted option): {correct_ans}\n")
                    # If the predicted correct answer (out of ans1, ans2, ans3, ans4)
                    # matches the actual correct answer (CorrectAnswer), then update
                    # the variables accordingly. If there is no match, then update
                    # the variables accordingly.
                    cleaned_predicted_answer = removepun(qna["Answers"][correct_ans-1])
                    cleaned_actual_answer = removepun(qna["CorrectAnswer"])
                    print(f'\033[1;33mPredicted Answer: {qna["Answers"][correct_ans-1]}\033[0m')
                    print(f'\033[1;33mActual Answer: {qna["CorrectAnswer"]}\033[0m')
                    if cleaned_predicted_answer == cleaned_actual_answer:
                        print('\033[1;32mCorrect Match\033[0m')
                        logging.info('Correct Match')
                        corr_qna.append((para_no, qna["Question"], qna["CorrectAnswer"]))
                        print((para_no, qna["Question"], qna["CorrectAnswer"]))
                        logging.info((para_no, qna["Question"], qna["CorrectAnswer"]))
                        matchedques.append((int(para_no),count))
                        correct_score.append(max_score)
                        correct_match.append(curr_corr)
                        paras.append(para_no)
                        edge_match.append((int(para_no),count))
                        edge_qtype.update({qtypes[int(para_no) - 1] : edge_qtype.get(qtypes[int(para_no) - 1], 0) + 1})
                        pred_dict.update({qtypes[int(para_no) - 1] : pred_dict.get(qtypes[int(para_no) - 1], 0) + 1})
                    else:
                        print('\033[1;31mIncorrect Match\033[0m')
                        logging.info('Incorrect Match')
                        incorr_qna.append((para_no, qna["Question"], qna["Answers"][correct_ans-1], qna["CorrectAnswer"]))
                        print((para_no, qna["Question"], qna["Answers"][correct_ans-1], qna["CorrectAnswer"]))
                        logging.info((para_no, qna["Question"], qna["Answers"][correct_ans-1], qna["CorrectAnswer"]))
                        wrong.append([para_no, count, qna["Question"], qna["CorrectAnswer"], qna["Answers"][correct_ans - 1]])
                        wrong_score.append(max_score)
                        wrong_match.append(curr_corr)
                        wrong_paras.append(para_no)
                    print("----------------------------------------------------------------")
                    logging.info("----------------------------------------------------------------")
                count += 1
    logging.info('End of Logs for Correct and Wrong Match Analysis')
    print('\033[1;32mCorrect and Wrong Match Analysis performed successfully\033[0m')
    return {
            "edge_match": edge_match,
            "pred_dict": pred_dict,
            "edge_qtype": edge_qtype,
            "matchedques": matchedques,
            "wrong": wrong,
            "curr_corr": curr_corr,
            "corr_qna": corr_qna,
            ...
            "wrong_paras": wrong_paras
            }

def analyze_output(matchedques: list, edge_match: list, 
                   edge_qtype: dict, corr_qna: list, 
                   wrong_paras: list, incorr_qna: list, 
                   wrong_match: list, wrong_score: list) -> pd.DataFrame:
    """
    ...
    print(f"The total Questions and Answers matched till edges are: {len(matchedques)}")
    logging.info(f"The total Questions and Answers matched till edges are: {len(matchedques)}")

    print(f"The correctly matched questions are:\n{matchedques}\n")
    logging.info(f"The correctly matched questions are:\n{matchedques}\n")

    print(f"The No. of questions matched using edges are: {len(edge_match)}")
    logging.info(f"The No. of questions matched using edges are: {len(edge_match)}")
    # since it is calulated first , edge_match and matchedques will be same

    print(f"The types of question predicted till edges are:\n{edge_qtype}")
    logging.info(f"The types of question predicted till edges are:\n{edge_qtype}")
    ...
    unmatched_df = pd.DataFrame(wrong_match,
                                columns = ["nsubj", "dobj", "vprep", "nprep",
                                        "verb", "prep"," adjv", "subj_obj",
                                        "noun", "noun chunks"])
    unmatched_df.insert(0, "Question No.", wrong_paras, True)
    unmatched_df["wrongly predicted total score"] = wrong_score
    print(f"Unmatched DF:\n{unmatched_df}\n")
    logging.info(f"Unmatched DF:\n{unmatched_df}\n")
    print('\033[1;32mOutput analysed successfully\033[0m')

    return unmatched_df

def main():
    df, all_sent, _, qtypes, mcq, finalmcq, finalpara, adjustedIndex = load_required_files_and_display_data()\
    # Load the model Model
    model1 = "bert-base-nli-mean-tokens"
    model2 = "all-mpnet-base-v2"
    model3 = "all-distilroberta-v1"

    choice = int(input('Enter:\n\'0\' for the bert-base-nli-mean-tokens model\
    \n\'1\' for the all-mpnet-base-v2 model\
    \n\'2\' for the all-distilroberta-v1 model\
    \n\'3\' for another model you want\
    \nEnter your choice: '))
    match choice:
        case 0: model_name = model1
        case 1: model_name = model2
        case 2: model_name = model3
        case 3: model_name = input('Copy-paste the name of the model: ')
    ...
    output_dict = correct_and_wrong_match_analysis(qtypes, mcq, finalmcq, finalpara, all_sent, adjustedIndex, model)

    unmatched_df = analyze_output(output_dict["matchedques"], 
                                  output_dict["edge_match"],
                                  output_dict["edge_qtype"],
                                  output_dict["corr_qna"],
                                  output_dict["wrong_paras"],
                                  output_dict["incorr_qna"],
                                  output_dict["wrong_match"],
                                  output_dict["wrong_score"])
    ...
    logging.info(f'''================================================================================
                ...
                \n                Number of questions predicted correctly: {len(output_dict["matchedques"])}
                \n               Number of questions predicted incorrectly: {len(output_dict["wrong_paras"])}
                \n                        ACCURACY: {(len(output_dict["matchedques"]) / len(all_sent)) * 100}
                \n                            ERROR: {(len(output_dict["wrong_paras"]) / len(all_sent)) * 100}
    ...
    "{date}_{cur_time}___sci_incorrectmatch_list1-for310___sci4_reorder_edge_cosine-310___{model_name}.xlsx"')

    # Save the Excel Sheet of Incorrectly Matched Questions
    # Format: Datetime___Filename___Codename___Modelname.xlsx
    unmatched_df.to_excel(f"{date}_{cur_time}___sci_incorrectmatch_list1-for310___sci4_reorder_edge_cosine-310___{model_name}.xlsx")

    try:
        shutil.copy(f"{date}_{cur_time}___sci_incorrectmatch_list1-for310___sci4_reorder_edge_cosine-310___{model_name}.xlsx",
                    "./data/incorrect_matches")
        os.remove(f"./{date}_{cur_time}___sci_incorrectmatch_list1-for310___sci4_reorder_edge_cosine-310___{model_name}.xlsx")
    ...
    try:
        ...
        shutil.copy('./log_file.log',
                    f'./logs/{date}_{cur_time}___sci4_reorder_edge_cosine-310___{model_name}___logfile.log')
    ...
    print(f'''\033[1m================================================================================\033[0m
            ...
            \n\033[1m================================================================================\033[0m
            \n\033[1m================================================================================\033[0m
            \n\033[1;33m                         MODEL NAME: {model_name}\033[0m
            \n\033[1;33m                       Total number of questions: {len(all_sent)}\033[0m
            \n\033[1;32m                Number of questions predicted correctly: {len(output_dict["matchedques"])}\033[0m
            \n\033[1;31m                Number of questions predicted incorrectly: {len(output_dict["wrong_paras"])}\033[0m
            \n\033[1;32m                            ACCURACY: {(len(output_dict["matchedques"]) / len(all_sent)) * 100}\033[0m
            \n\033[1;31m                             ERROR: {(len(output_dict["wrong_paras"]) / len(all_sent)) * 100}\033[0m
